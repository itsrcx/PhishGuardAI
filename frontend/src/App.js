...
function App() {
  ...
  const scanUrl = async () => {
    ...
    try {
      ...
      // API Gateway Cognito Authorizers often expect the Access Token for resource authorization,
      // especially if custom scopes are involved. The ID Token is primarily for user authentication.
      // If you continue to face authorization issues with the ID Token, consider using `auth.user?.access_token` instead.
      const token = auth.user?.access_token; // Or auth.user?.access_token
